...
const HOLD_MS = 2500;     // the spec's pause, before anything moves
const DRIFT_MS = 950;
...
export default function PastReveal({ onDone }: { onDone?: () => void }) {
  ...
  useEffect(() => {
    if (!armed) return;
    const timers: number[] = [];
    CHARS.forEach((_, i) => timers.push(window.setTimeout(() => setTyped(i + 1), i * TYPE_MS)));
  ...
  return (
    <div ref={rootRef} className="relative w-full">
      {/* The sentence. Goes absolute the moment the diagonal takes over, so it
          drops out of the flow and fades while the letters fly out of it. */}
      <div
        aria-hidden={diagonal}
        style={{
          position: diagonal ? 'absolute' : 'static',
          top: 0, left: 0, right: 0,
          opacity: diagonal ? 0 : 1,
          transition: 'opacity 420ms ease-out',
          pointerEvents: 'none',
        }}
      >
        <p className="font-serif leading-snug" style={{ fontSize: 'clamp(23px, 6.2vw, 34px)', color: 'var(--th-surface)', maxWidth: '17ch' }}>
          We break down context by using the{' '}
      ...
      {diagonal && (
        <div className="w-fit">
          {LENSES.map((lens, i) => (
            <div key={lens.key} className="flex items-baseline" style={{ marginLeft: `${(i + 1) * 13}%` }}>
              <span
                ref={(el) => { bigRefs.current[i] = el; }}
                className="font-display leading-none"
                style={{ color: lens.colour, fontSize: 'clamp(46px, 15vw, 92px)', opacity: 0, willChange: 'transform', display: 'inline-block' }}
              >
                {lens.label[0]}
              </span>
              <span
                className="font-display leading-none"
                style={{
                  color: lens.colour, fontSize: 'clamp(20px, 6vw, 36px)',
      ...
      <p
        ...
        style={{
          fontSize: 'clamp(18px, 4.8vw, 24px)', color: 'var(--th-surface)', maxWidth: '24ch',
          opacity: tail ? 0.92 : 0,
          transform: tail ? 'translateY(0)' : 'translateY(10px)',
          transition: 'opacity 700ms ease-out, transform 700ms ease-out',
        }}
      >
        These help you <strong>ASK</strong> about the world through big picture lenses.
      </p>
    </div>
  );
}
