...
export async function researchDraft(system: string, userText: string): Promise<ResearchOutput | null> {
  ...
  // web_search_20260209 filters its results by running code in a container. Any turn that
  // resends assistant content containing those tool uses must hand the container back, or
  // the API 400s: "container_id is required when there are pending tool uses generated by
  // code execution with tools". That killed every pushback below until we carried this.
